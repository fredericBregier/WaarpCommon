 ...
 * <p>
 * It computes the statistics for both inbound and outbound traffic periodically at the given {@code checkInterval}, and
 * calls the {@link AbstractTrafficShapingHandler#doAccounting(TrafficCounter)} method back. If the {@code
 * checkInterval} is {@code 0}, no accounting will be done and statistics will only be computed at each receive or write
 * operation.
 * </p>
 */
public class TrafficCounter {
    ...
    /**
     * Constructor with the {@link AbstractTrafficShapingHandler} that hosts it, the {@link ScheduledExecutorService} to
     * use, its name, the checkInterval between two computations in milliseconds.
     *
     * @param executor the underlying executor service for scheduling checks, might be null when used from {@link
    ...
    /**
     * Constructor with the {@link AbstractTrafficShapingHandler} that hosts it, the Timer to use, its name, the
     * checkInterval between two computations in millisecond.
     *
    ...
    /**
     * @return the lastCumulativeTime in millisecond as of System.currentTimeMillis() when the cumulative counters were
     * reset to 0.
     */
    ...
    /**
     * Reset both read and written cumulative bytes counters and the associated absolute time from
    ...
    public String toString() {
        return new StringBuilder(165).append("Monitor ").append(name)
                                     ...
                                     .append("Current asked Write: ").append(currentWrittenBytes.get() >> 10)
                                     .append(" KB, ")
                                     .append("Current real Write: ").append(realWrittenBytes.get() >> 10).append(" KB")
                                     .toString();
    }
