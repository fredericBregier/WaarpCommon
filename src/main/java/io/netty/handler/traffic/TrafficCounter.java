 ...
 * at the given {@code checkInterval}, and
 * calls the {@link AbstractTrafficShapingHandler#doAccounting(TrafficCounter)}
 * method back. If the {@code
 * checkInterval} is {@code 0}, no accounting will be done and statistics will
 * only be computed at each receive or write
 * operation.
 * </p>
 */
public class TrafficCounter {
   ...
   * the {@link ScheduledExecutorService} to
   ...
   *     might be null when used from {@link
   ...
   *     computations.
   */
  public TrafficCounter(ScheduledExecutorService executor, String name,
                        long checkInterval) {
    if (name == null) {
  ...
  public synchronized void start() {
    ...
    if (localCheckInterval > 0 && executor != null) {
      ...
      scheduledFuture =
          executor.scheduleAtFixedRate(monitor, 0, localCheckInterval,
  ...
  synchronized void resetAccounting(long newLastTime) {
    ...
    if (logger.isDebugEnabled() && interval > checkInterval() << 1) {
      logger.debug(
          "Acct schedule not ok: " + interval + " > 2*" + checkInterval() +
          " from " + name);
    }
   ...
   *     when the cumulative counters were
   *     reset to 0.
   */
   ...
   * given limitTraffic and the max wait
   * time.
   *
   ...
   */
  @Deprecated
  public long readTimeToWait(final long size, final long limitTraffic,
                             final long maxTime) {
   ...
   */
  public long readTimeToWait(final long size, final long limitTraffic,
                             final long maxTime, final long now) {
    ...
    if (interval > AbstractTrafficShapingHandler.MINIMAL_WAIT) {
      ...
      if (time > AbstractTrafficShapingHandler.MINIMAL_WAIT) {
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Time: " + time + ':' + sum + ':' + interval + ':' + pastDelay);
        }
    ...
    if (time > AbstractTrafficShapingHandler.MINIMAL_WAIT) {
      if (logger.isDebugEnabled()) {
        logger.debug(
            "Time: " + time + ':' + lastsum + ':' + lastinterval + ':' +
            pastDelay);
      }
      if (time > maxTime && now + time - localReadingTime > maxTime) {
        time = maxTime;
   ...
   */
  @Deprecated
  public long writeTimeToWait(final long size, final long limitTraffic,
                              final long maxTime) {
