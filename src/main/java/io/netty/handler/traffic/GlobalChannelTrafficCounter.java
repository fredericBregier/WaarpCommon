 ...
 * is the Global one, and its special
 * property is to directly handle other channel's TrafficCounters. In
 * particular, there are no scheduler for those
 * channel's TrafficCounters because it is managed by this one.
 */
public class GlobalChannelTrafficCounter extends TrafficCounter {
  /**
   * @param trafficShapingHandler the associated {@link
   *     GlobalChannelTrafficShapingHandler}.
   * @param executor the underlying executor service for scheduling checks
   *     (both Global and per Channel).
  ...
  public synchronized void start() {
    ...
    if (localCheckInterval > 0) {
      monitorActive = true;
      monitor = new MixedTrafficMonitoringTask(
  ...
  public void resetCumulativeTime() {
    for (PerChannel perChannel :
        ((GlobalChannelTrafficShapingHandler) trafficShapingHandler).channelQueues
            .values()) {
   ...
   * way it mixes Global and Channel
   * counters.
   */
  private static class MixedTrafficMonitoringTask implements Runnable {
    ...
    /**
     * @param trafficShapingHandler The parent handler to which this task
     *     needs to callback to for accounting.
    ...
    public void run() {
      ...
      for (PerChannel perChannel : trafficShapingHandler1.channelQueues
          .values()) {
