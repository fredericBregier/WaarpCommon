 ...
 * path. Used as a destination in {@code
...
public final class HttpResourceModel {
   ...
   * httprequest, Object that contains the method.
   *
  ...
  /**
   * Gathers all parameters' annotations for the given method, starting from the
   * third parameter.
   */
  private List<Map<Class<? extends Annotation>, ParameterInfo<?>>> createParametersInfos(
    ...
    for (int i = 2; i < parameterAnnotations.length; i++) {
      ...
      int presence = 0;
      for (Class<? extends Annotation> annotationClass : paramAnnotations
          .keySet()) {
  ...
  private Object getQueryParamValue(
    ...
    ParameterInfo<List<String>> info =
  ...
  /**
   * Returns a List of String created based on the {@link DefaultValue} if it is
   * presented in the annotations Map.
   *
