 ...
 * routing URI paths to http resources.
 * Parameters within braces "{}" are treated as template parameter (a named
 * wild-card pattern).
 *
...
public final class PatternPathRouterWithGroups<T> {
   ...
   *
   * @param source Source path to be routed. Routed path can have named
   *     wild-card pattern with braces "{}".
  ...
  /**
   * Get a list of destinations and the values matching templated parameter for
   * the given path. Returns an empty list
   * when there are no destinations that are matched.
   *
  ...
  public List<RoutableDestination<T>> getDestinations(String path) {
    ...
    for (ImmutablePair<Pattern, RouteDestinationWithGroups> patternRoute : patternRouteList) {
      ...
      if (matcher.matches()) {
        ...
        result.add(
            new RoutableDestination<T>(
                patternRoute.getSecond().getDestination(),
                groupNameValuesBuilder));
      }
    }
    return result;
  }
  ...
  public static final class RoutableDestination<T> {
    ...
    /**
     * @return Map of templated parameter and string representation group value
     *     matching the templated parameter as
     *     the value.
     */
