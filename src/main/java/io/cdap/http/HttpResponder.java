...
public interface HttpResponder {
  ...
  /**
   * Sends json response back to the client. This is a convenient method to send
   * json encoded string with content type
   ...
   * "application/octet-stream", but can be overridden
   ...
   * body can be sent in chunks using the
   ...
   * "application/octet-stream", but can be overridden by the
   * headers parameter.
   *
   * @param status Status of the response.
   ...
   * {@link BodyProducer}. This method will
   * return immediate after it is called. Invocation of methods on the given
   * {@link BodyProducer} will be triggered
   * from another thread. Default content type is "application/octet-stream",
   * but can be overridden by the headers
   * parameter.
   *
   * @param status Status of the response.
