 ...
 * character but
 * should not throw an ArrayIndexOutOfBoundsException either. Led to discovery of mishandling (or potential for better
 ...
 * encoding
 * (by padding with equals signs) too soon. Also added an option to suppress the automatic decoding of gzipped streams.
 * Also added experimental support for specifying a class loader when using the {@link #decodeToObject(java.lang.String,
 * int, java.lang.ClassLoader)} method.</li>
 ...
 * that the
 * code doesn't have to create two output arrays: an oversized initial one and then a final, exact-sized one. Big win
 * when using the {@link #encodeBytesToBytes(byte[])} family of methods (and not using the gzip options which uses a
 * different mechanism with streams and stuff).</li>
 ...
 * and
 * finally executed. Thanks to everyone who sent me stuff, and I'm sorry I wasn't able to distribute your fixes to
 ...
 * permit the
 * GZIP option) use IO streams, there is a possiblity of an java.io.IOException being thrown. After some discussion and
 ...
 * are cleaner. You'll probably have to change some method calls that you were making to support the new options format
...
public class Base64 {
     ...
     * It is important to note that data encoded this way is <em>not</em> officially valid Base64, or at the very least
     * should not be called Base64 without also specifying that is was encoded using the URL- and Filename-safe
     * dialect.
     */
     ...
     * java.io.IOException.
     * <b>This is new to v2.3!</b> In earlier versions, it just returned a null value, but in retrospect that's a pretty
     * poor way
     * to handle it.
     * </p>
     * <p>
     ...
     * the <var>source</var> array or <var>destOffset</var> + 3 for the <var>destination</var> array. This method
