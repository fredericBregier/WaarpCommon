...
public abstract class PassthroughBasedFileImpl extends AbstractFile {
  ...
  /**
   * DirInterface associated with this file at creation. It is not necessary the
   ...
   * therefore no limitation of the file size to
   * 2^32 bytes.
   * <p>
   * The returned block is limited to sizeblock. If the returned block is less
   * than sizeblock length, it is the last
   * block to read.
   *
  ...
  /**
   * End the Write of the current FileInterface with the given ByteBuf. The file
   * is not limited to 2^32 bytes since
   ...
   * limited to 2^32 bytes since this write
  ...
  /**
   * Return the current position in the FileInterface. In write mode, it is the
   * current file length.
   *
   * @return the position
   */
