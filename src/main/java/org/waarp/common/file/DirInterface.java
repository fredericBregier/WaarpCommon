...
public interface DirInterface {
  ...
  /**
   * Construct and Check if the given path is valid from business point of view
   * (see {@link AuthInterface})
   *
   * @param path
   *
   * @return the construct and validated path (could be different than the one
   *     given as argument, example: '..' are
   *     removed)
   *
  ...
  /**
   * List all files with other informations from the given path (could be a file
   * or a directory)
   *
   * @param path
  ...
  /**
   * Give for 1 file all informations from the given path (could be a file or a
   * directory)
   *
   * @param path
   * @param lsFormat True if ls Format, else MLSx format
