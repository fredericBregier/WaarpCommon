 ...
 * keyStore. To generate those files, you can
 * use the "keytool" command from the JDK or using the free tool KeyTool IUI
 ...
 * <br>
 * <p>
 * Usage:<br> In order to use the SSL support, here are the different
 * steps.<br>
 * <br>
 *
 ...
 * needClientAuthent);<br> Note:
 * needClientAuthent is True if the TrustStore is used to authenticate Clients,
 * False if only to authenticate
 * Servers</li>
 ...
 * As first item in the pipeline, add:<br> pipeline.addLast("ssl",
 * ggSslContextFactory.initInitializer(<b>false</b>,
 * ggSslContextFactory.hasTrustStore(), executor));<br> where executor is
 * generally a
 * Executors.newCachedThreadPool();<br>
 * <br>
 * <p>
 * For example, see Waarp Local Exec module using SSL:<br>
 * localExecClientInitializer = new
 * LocalExecSslClientInitializer(ggSslContextFactory);<br>
 ...
 * e)<br> throws Exception {<br> ...<br>
 * SslHandler sslHandler = ctx.pipeline().get(SslHandler.class);<br> // Begin
 * handshake<br> ChannelFuture
 * handshakeFuture = sslHandler.handshake();<br> handshakeFuture.addListener(new
 * ChannelFutureListener() {<br> public
 * void operationComplete(ChannelFuture future)<br> throws Exception {<br> if
 * (future.isSuccess()) {<br> //OK<br> } else
 ...
 * needClientAuthent);<br> Note:
 * needClientAuthent is True if the TrustStore is used to authenticate Clients,
 * False if only to authenticate Servers
 * <li>
 * <li>Create the WaarpSslContextFactory:<br>
 * WaarpSslContextFactory ggSslContextFactory = new WaarpSslContextFactory(ggSecureKeyStore,
 * <b>true</b>);</li>
 * <li>Create your own Initializer:<br>
 * As first item in the pipeline, add:<br> pipeline.addLast("ssl",
 * ggSslContextFactory.initInitializer(<b>true</b>,
 * ggSslContextFactory.hasTrustStore(), executor));<br> where executor is
 * generally a
 * Executors.newCachedThreadPool();<br>
 * <br>
