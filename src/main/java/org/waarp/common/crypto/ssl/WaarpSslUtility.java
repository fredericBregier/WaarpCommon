...
public class WaarpSslUtility {
   ...
   *
   * @param future might be null, condition to start to add the handler to
   *     the pipeline
   * @param pipeline
   * @param sslHandler
   ...
   * when handler is added after channel is
   * active)
   *
   * @param channel
   *
  ...
  public static boolean waitForHandshake(Channel channel) {
    ...
    if (handler instanceof SslHandler) {
      ...
      logger.error(
          "SSL Not found but connected: " + handler.getClass().getName());
      return true;
    }
  }
   ...
   *
   * @param future if not null, wait for this future to be done to removed
   *     the sslhandler
   * @param channel
