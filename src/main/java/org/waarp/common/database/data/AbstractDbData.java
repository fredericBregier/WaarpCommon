 ...
 * If the connection is not in autocommit,
...
public abstract class AbstractDbData {
  ...
  /**
   * To setup primaryKey, otherFields, allFields. Note this initObject is called
   * within constructor of AbstractDbData.
  ...
  public void select() throws WaarpDatabaseException {
    ...
    try {
      preparedStatement
          .createPrepareStatement("SELECT " + getSelectAllFields() +
                                  " FROM " + getTable() + " WHERE " +
                                  getWherePrimaryKey());
      setPrimaryKey();
      setValues(preparedStatement, primaryKey);
