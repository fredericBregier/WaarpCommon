...
public class DbAdmin {
  protected static final Timer dbSessionTimer =
      new HashedWheelTimer(new WaarpThreadFactory("TimerClose"),
   ...
   */
  private static ConcurrentHashMap<UUID, DbSession> listConnection =
   ...
   * database for the scheme that provides
   ...
   * DbSession objects that could be created (=> need a hashtable for specific
   * driver when created). Also, don't know
   * if two drivers at the same time (two different DbSession) is allowed by
   * JDBC.
   *
   * @param model
   * @param server
   ...
   * the constructor DbAdmin(Connection,
   * boolean).
   */
  public void close() {
    if (getSession() != null) {
