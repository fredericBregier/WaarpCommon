...
public class XmlValue {
  ...
  /**
   * @return the associated SubXML with the XmlValue (might be null if singleton
   *     or Multiple)
   */
  ...
  /**
   * @return the associated list with the XmlValues (might be null if singleton
   *     or SubXml)
   */
  public List<?> getList() {
    return this.values;
  ...
  /**
   * Convert String value to the specified type. Throws InvalidArgumentException
    ...
    try {
      ...
      //
        ...
        return new java.sql.Date(
            XmlStaticShared.dateFormat.parse(value).getTime());
        ...
        // Should not be
        return new java.sql.Date(
            XmlStaticShared.timeFormat.parse(value).getTime());
      } else {
   ...
   * subnodes are present but not if those nodes
   * are empty.
   *
