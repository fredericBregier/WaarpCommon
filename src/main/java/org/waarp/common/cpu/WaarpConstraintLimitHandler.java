...
public abstract class WaarpConstraintLimitHandler implements Runnable {
  ...
  private final LinkedList<CurLimits> curLimits =
      new LinkedList<WaarpConstraintLimitHandler.CurLimits>();
   ...
   * proportionally to the current Load (if CPU
   * related)
   *
  ...
  /**
   * Check every delay if the current cpu usage needs to relax or to constraint
   * the bandwidth
   */
  public void run() {
  ...
  /**
   * Get the current setting on Read Limit (supposed to be not the value in the
   * handler but in the configuration)
   *
  ...
  /**
   * Get the current setting on Write Limit (supposed to be not the value in the
   * handler but in the configuration)
   *
