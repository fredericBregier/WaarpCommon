...
public class WaarpNettyUtilTest {
    ...
    public void setServerBootstrap() {
        ...
        try {
            ...
            Channel clientChannel = clientBootstrap.connect("localhost", PORT).sync().channel();
    ...
    public void setServerSslBootstrap1Group() throws CryptoException {
        ...
        WaarpSecureKeyStore WaarpSecureKeyStore =
                new WaarpSecureKeyStore(file.getAbsolutePath(), keyStorePasswd, keyPassword);
        // Include certificates
        String trustStoreFilename = "certs/testcert.jks";
        File file2 = new File(classLoader.getResource(trustStoreFilename).getFile());
        assertTrue("File2 Should exists", file2.exists());
        String trustStorePasswd = "testcert";
        WaarpSecureKeyStore.initTrustStore(file2.getAbsolutePath(), trustStorePasswd, true);
    ...
    public class ExtendedConstraintLimitHandler extends WaarpConstraintLimitHandler {
    ...
    public void setServerBootstrap1GroupWithConstraint() {
        final EventLoopGroup workerGroup = new NioEventLoopGroup();
        final GlobalTrafficShapingHandler trafficShapingHandler = new GlobalTrafficShapingHandler(workerGroup.next());
        final ExtendedConstraintLimitHandler constraintLimitHandler = new ExtendedConstraintLimitHandler(trafficShapingHandler);
